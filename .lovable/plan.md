...
Rebuild the newer uploaded prototype (`index_2.html`) as a real React app, keeping its exact look (brand blue, Inter, slate surfaces, rounded cards) and its rules-based logic unchanged. Add the project development report as a proper page in the app.

## Pages

- `/` — landing: title, subtitle, disclaimer, "What this tool does" / "does not do" cards, core question box, Start prioritisation + "Try a realistic example" buttons, collapsible "How this works" with the scoring logic.
- `/prioritise` — the 3-step wizard with the progress indicator:
  ...
  2. Rate initiatives: only initiatives mapped to selected problems, sticky selected-problem chips, Impact and Effort choices, rationale shown inline, "Linked to your selected problems" details, incomplete-rating validation.
  3. Results: grouped priority bands (Start here / Medium / Lower), score per initiative, "Suggested next steps" panel for the top 2 "Start here" initiatives, Impact–Effort matrix, results disclaimer, copy-summary, print / save-as-PDF, restart.
...
- Problem→initiative mapping, initiative names/descriptions/rationales, next-step suggestions per initiative, and keyword suggestion lists copied verbatim from `index_2.html` (no AI, no backend).
- "Try a realistic example" loads the mid-size discrete manufacturer persona: modal with the scenario story, preset problems (visibility, inventory, OTIF) and preset ratings, then jumps to the initiatives step.
- Print styles: results only, hiding nav/progress/buttons, with the "Generated by P2I" print footer.
